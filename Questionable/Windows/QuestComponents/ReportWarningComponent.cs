...
internal sealed class ReportWarningComponent(Configuration configuration)
{
    ...
    {
        ImGui.TextColored(QstTheme.Danger, _L("Future message"));
        ...
        if (ImGuiComponentsLocal.IconButtonWithText(FontAwesomeIcon.ExclamationTriangle, _L("Opt Out"), QstTheme.Accent))
        {
        ...
        ImGui.SameLine();
        if (ImGuiComponentsLocal.IconButtonWithText(FontAwesomeIcon.ExclamationTriangle, _L("Dismiss"), QstTheme.Success))
            _configuration.General.DismissedReportWarning = true;
    }
}
